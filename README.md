...
Secrets are read from stdin, not command arguments. The dashboard and CLI save them to `~/.config/gighunt/.env.local` (or `$GIGHUNT_CONFIG_DIR/.env.local`) with owner-only permissions. Back up this file securely. Existing values in the older `secrets.json` file or OS keychain remain readable; new saves go to `.env.local`. Environment variables override stored secrets and cannot be changed from the dashboard. The repository's `.env.example` is only a reference, and a repository-root `.env.local` is not loaded automatically; use the dashboard or CLI to store keys in the config directory. **Restart the daemon after changing credentials through the CLI or environment**; dashboard changes reload the provider clients automatically.
